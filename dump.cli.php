...
echo "All Table and Rows Saved into array!\n";

foreach ($tables as $key => $table) 
{
	preg_match("/`[a-zA-Z0-9]+`/", $table, $matches);
	$t_name = str_replace('`','',$matches[0]);
	echo "Creating Table $t_name\n";
	$query = $db -> prepare($table);
	$query -> execute();
	$track = $query->errorInfo();
	if($track[0]=='00000') echo "Table Created successfully\n";
	else echo $track[2]."\n";
	...
	if(count($rows[$t_name])>0)
	{
		...
		$query -> execute();
		echo $query->rowCount(). " Row Inserted In Table '$t_name'\n";
	}
}
$timeNeeded = microtime(true) - $start;
echo "Complited in ".$timeNeeded." Second\n";
?>